...
drop constraint if exists reactions_star_id_sender_user_id_key;

drop index if exists public.idx_reactions_unique_regular_per_star_sender;
drop index if exists public.idx_reactions_unique_special_per_star_sender;

create unique index if not exists idx_reactions_unique_regular_per_star_sender
on public.reactions (star_id, sender_user_id)
where reaction_type_id not in (5, 6);

create unique index if not exists idx_reactions_unique_special_per_star_sender
on public.reactions (star_id, sender_user_id)
...
begin
  ...
  select
    ...
    (
      ...
      and (
        ...
        or not coalesce(viewer_reaction.has_special_reaction, false)
      )
    ) as is_reactable,
  ...
  left join lateral (
    select
      ...
      bool_or(r.reaction_type_id in (5, 6)) as has_special_reaction
    from public.reactions r
  ...
  group by
    ...
    viewer_reaction.has_special_reaction;
exception
